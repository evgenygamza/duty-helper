...
- [ ] `slack login` в песочнице
- [ ] `slack init` в `bot/` — появятся `.slack/config.json` и `.slack/hooks.json`
- [ ] `slack app link` к уже созданному приложению песочницы
- [ ] манифест из репозитория: `manifest.source: local`
- [ ] два контура одним генератором на хуке `get-manifest`: песочница
      с запасом, прод по минимуму
...
Сейчас упирается в `slack login` — шаг интерактивный, нужен человек.
